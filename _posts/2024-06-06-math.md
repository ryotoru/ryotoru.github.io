...
Currently, most of the cryptography used in our daily lives relies on protocols like RSA and Diffie-Hellman. RSA is based on the mathematical assumption that factoring a very large number into its prime factors is computationally difficult. This hardness is what secures RSA encryption, making it feasible to encrypt data with a public key while only the corresponding private key, which is derived from these prime factors, can decrypt it.

Diffie-Hellman, on the other hand, is based on the discrete logarithm problem. This problem assumes that given a number \(y\), which is calculated as:
...
I will delve deeper into these topics in the next post.
